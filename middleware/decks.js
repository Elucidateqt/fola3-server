...
exports.isOwnerOrManager = async (req, res, next) => {
    ...
    if (req.locals.deck.public === false) {
        const notAllowed = !req.locals.user._id.equals(req.locals.deck.owner._id) && !req.locals.user.effectivePermissions.some(permission => permission.name ==='API:DECKS:MANAGE')
        if(notAllowed){
