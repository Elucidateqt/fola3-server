...
router.post('/:setId', authMiddleware.authenticateToken, authMiddleware.authenticatePermission('API:CARDSETS:MANAGE'),
param('setId').trim().isUUID().withMessage('must be valid UUID'),
body('owner').optional().trim().custom(cardSetMiddleware.isOwnerParamValid),
body('public').optional().isBoolean(),
body('name').optional().isString().trim().isLength({min: 3, max: 128}).withMessage('Must be between 3 and 128 characters long.').isAlphanumeric('en-US', {ignore: ' '}).withMessage('error.no_special_characters'),
cardSetMiddleware.loadCardSet,
